...
20. - [x] Include the OpenFastTrace logo as application icons in the binary package (`dsn~app-icon~1`).
21. - [x] Declare the package icon using AppStream metadata and a desktop file (`dsn~package-icon-declaration~1`, `req~package-icon-declaration~1`).
22. - [x] Declare the application screenshot in the AppStream metadata using remote URL (`dsn~package-screenshots~1`, `req~package-screenshots~1`).

## Phase 5: Integration Testing Automation
23. - [ ] Implement `test-packaging.sh` to automate the full build process and verify the resulting files (`dsn~integration-testing~1`).
24. - [ ] Verify that `test-packaging.sh` calls and validates the output of `check-preconditions.sh` (`dsn~precondition-check~1`).
25. - [ ] Verify that `test-packaging.sh` correctly tests the integration of `create-source-package.sh` and `create-binary-package.sh` (`dsn~build-orchestration-scripts~1`).
26. - [ ] Verify that `test-packaging.sh` checks for all expected Debian artifacts (`dsn~integration-testing~1`).
27. - [ ] Verify that `test-packaging.sh` runs `shellcheck` on all shell scripts (`dsn~integration-testing~1`, `dsn~finding-free-shellcheck~1`).

## Phase 6: Quality Assurance
28. - [ ] Perform a manual code review to ensure adherence to clean code principles (`dsn~clean-code-principles~1`).

## Phase 7: Final Verification
29. - [ ] Execute the full packaging workflow for a known OpenFastTrace release version.
30. - [ ] Verify the resulting `.deb` package metadata and file contents.
31. - [ ] Verify that the `debian/changelog` is correctly appended and preserves historical entries.
